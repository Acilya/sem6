...
void OnKeyPress(unsigned char key, int x, int y) {        // key - kod ASCII wcisnietego klawisza, x i y - wspolrzedne polozenia kursora myszy
	player.speed = .1f;
	printf("KeyPress: %c\n", key);
	if (!keystate[key]) {
...
void OnTimer(int id) {
	...
	// Chodzenie na boki: p' = p + e * s, gdzie e prostopadle do d = [x, y], e = [y, -x] lub [-y, x]
	if (keystate['a'])
	{
	...
	// Obracanie sie: 
	double alfa = 0.02, beta;
	if (keystate['q'])
	{
		beta = atan2(player.dir.z, player.dir.x);
		player.dir.x = cos(-alfa + beta);
		player.dir.z = sin(-alfa + beta);
	}
	if (keystate['e'])
	{
		beta = atan2(player.dir.z, player.dir.x);
